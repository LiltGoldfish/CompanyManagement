...
<html>
					...
					<div class="am-cf am-padding am-padding-bottom-0">
						<div class="am-fl am-cf">
							<strong class="am-text-primary am-text-lg"> </strong><small>
								<i class="icon-home"></i> &gt; 首页 &gt; 绩效信息管理</small>
						</div>
					</div>
					<hr>
					<div class="container">
  ...
  if(isset($_POST["flag"]))
   { 
   	  ...
   	  $s = $_SESSION['id'];
 	  $sqlstr ="select 绩效信息.id,姓名,部门名称,提交时间,内容 from 绩效信息,职工,部门 where 绩效信息.职工编号=职工.职工编号 and 职工.部门编号=部门.部门编号 and 职工.职工编号 = '$s' ";
      $result = mysqli_query($conn,$sqlstr);
		...
		$sqlstr ="select 绩效信息.id,姓名,部门名称,提交时间,内容 from 绩效信息,职工,部门 where 绩效信息.职工编号=职工.职工编号 and 职工.部门编号=部门.部门编号 and 姓名 like '%".$_POST['职工姓名']."%' ";
        $result = mysqli_query($conn,$sqlstr);
		while ($rows = mysqli_fetch_row($result)){
				echo "<tr>";
				echo "<td><input name='chks' value='27' type='checkbox'></td>";
				for($i = 0; $i < count($rows); $i++){
					...
					echo "<td>
						 <div class='am-btn-toolbar'>
						 <div class='am-btn-group am-btn-group-xs'><button type='button' id='depart_26' class='am-btn am-btn-default am-fbtn-xs am-text-secondary btnedit'><span class='am-icon-pencil-square-o'></span> 编辑</button><button type='button' class='am-btn am-btn-default am-btn-xs am-text-danger amt-hide-sm-only' onclick='''><span class='am-icon-trash-o'></span> </span><a href = 'deletePerformance.php?id= ".$rows[0]." ' >删除</a></button></div>
						</div>
						</td>";
					echo "</tr>";
		}
